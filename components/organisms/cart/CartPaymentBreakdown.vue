<template>
  <div :class="paymentBreakdownClass">
    <AtomText :text="'お支払い内訳'" :classObject="paymentBreakdownTitleClass" />
    <div class="table w-full border-2 p-2">
      <div class="table-row-group">
        <div class="table-row">
          <div class="table-cell pt-4 pl-2 border-b-2">商品合計(税込)</div>
          <div
            class="table-cell text-right pr-2 border-b-2"
          >{{cart.goodsSubtotalIncludingTaxPrice | formatPriceWithUnit }}</div>
        </div>
        <div class="table-row">
          <div class="table-cell pl-2">梱包手数料(税込)</div>
          <div class="table-cell text-right pr-2">{{cart.packingFee | formatPriceWithUnit}}</div>
        </div>
        <div class="table-row">
          <div class="table-cell pl-2 border-b-2">送料(税込)</div>
          <div
            class="table-cell text-right pr-2 border-b-2"
          >{{cart.shippingFee | formatPriceWithUnit}}</div>
        </div>
        <div class="table-row">
          <div class="table-cell pl-2">小計(税込)</div>
          <div
            class="table-cell text-right pr-2"
          >{{cart.subtotalIncludingTaxPrice | formatPriceWithUnit}}</div>
        </div>
        <div class="table-row">
          <div class="table-cell pl-10 text-sm">非課税商品</div>
          <div class="table-cell text-right pr-2">{{cart.subtotalNonTaxable | formatPriceWithUnit}}</div>
        </div>
        <div class="table-row">
          <div class="table-cell pl-10 text-sm">内税対象額</div>
          <div class="table-cell text-right pr-2">{{cart.subtotalInternalTax | formatPriceWithUnit}}</div>
        </div>
        <div class="table-row">
        ...
        <div class="table-row">
          ...
          <div
            class="table-cell text-right pr-2"
          >{{cart.totalIncludingTaxPrice - cart.goodsSubtotalIncludingTaxPrice | formatPriceWithUnit}}</div>
        </div>
      </div>
    </div>
  </div>
...
export default Vue.extend({
  ...
  data() {
    return {
      paymentBreakdownTitleClass: {
        'text-lg': true,
      },
      paymentBreakdownClass: {
        'row-start-4': true,
        'col-start-9': true,
        'col-end-12': true,
      },
